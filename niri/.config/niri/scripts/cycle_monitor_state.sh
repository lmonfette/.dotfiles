...
NIRI_CFG_DIR=~/.config/niri
NIRI_STATE_DIR=$NIRI_CFG_DIR/state

INTEGRATED_MONITOR_ID="eDP-1"
SPARK_CENTER_MONITOR="DP-2"
...
set_monitor_state() {
    ...
    if [ "$STATE" = false ]; then
        ...
        niri msg output "$MONITOR_ID" off
        return 0
    fi

    # swaymsg output "$MONITOR_ID" enable
    niri msg output "$MONITOR_ID" on
}

# get_next_state() { cat $NIRI_STATE_DIR/next_monitor_state.txt; }
get_next_state() {  cat ${NIRI_STATE_DIR}/next_monitor_state.txt; }
...
set_next_state() {
    ...
    echo "$STATE_TO_SAVE" > ${NIRI_STATE_DIR}/next_monitor_state.txt
}
